...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Сразу же выполняем автоматический вход по свежим данным
      await authService.login({
        username: formData.email,
        password: formData.password,
      });

      // Успешная регистрация и вход — перенаправляем прямо в профиль
      router.push("/profile");
    } catch (err: any) {
      ...
      <div className="w-full max-w-md">
        <div className="bg-custom-bg-secondary rounded-3xl shadow-xl p-4 sm:p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="form-control">
              ...
              <div className="flex flex-wrap gap-4 gap-y-2">
                ...
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  <GraduationCap
                    size={18}
                    className="text-custom-main sm:size-5"
                  />
            ...
            {/* Отображение ошибки */}
            {error && (
              <div className="alert alert-error text-sm p-2">{error}</div>
            )}
